...
import { DELETE_BRANCH_RESET } from "../../constans/BranchConstans";
import { ToastContainer, toast } from 'react-toastify';
import { Link } from "react-router-dom";
import EditIcon from "@material-ui/icons/Edit";
const AllBranchs = ({ history }) => {
  ...
  const { error, branchs } = useSelector(
    (state) => state.branchs
  );
  const deleteBranchHandler = (branchId) => {
  ...
  const columns = [
    ...
    },
    {
      ...
      renderCell: (params) => {
        return (
          <div className="icon">
            <Link to={`/edit/branch/${params.getValue(params.id, "id")}`}>
              <EditIcon />
            </Link>
            <Button
              onClick={() =>
                deleteBranchHandler(params.getValue(params.id, "id"))
              }
            >
              <DeleteIcon />
            </Button>
